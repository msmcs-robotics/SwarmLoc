/**
 * RX Test v9c - Channel 2, 110kbps, 16MHz PRF
 *
 * Tests Channel 2 with different PLL settings vs Channel 5.
 * Ch2: FS_PLLCFG=0x08400508, FS_PLLTUNE=0x26
 * Ch5: FS_PLLCFG=0x0800041D, FS_PLLTUNE=0xBE
 *
 * Also adds FCG+PHE contradiction guard (found in v9b).
 *
 * PIN_RST = 7
 */
...
bool isStatusContradictory(uint32_t s) {
    ...
    if (fcg && phe) return true;   // Added: PHR error + frame good = impossible
...
void setup() {
    ...
    // Print PLL config for Channel 2
    byte pllcfg[4];
