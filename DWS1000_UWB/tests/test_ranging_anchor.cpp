...
void setup() {
    ...
    // Start as anchor with address "AA:BB:CC:DD:EE:FF:00:01"
    // Using long range mode for better reliability
    DW1000Ranging.startAsAnchor("AA:BB:CC:DD:EE:FF:00:01",
                                DW1000.MODE_LONGDATA_RANGE_LOWPOWER, false);

    // Re-apply LDO after configuration
    applyLDOTuning();

    Serial.println(F("Anchor ready, waiting for tags..."));
}
