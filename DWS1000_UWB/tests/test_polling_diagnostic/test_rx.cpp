...
void setup() {
    ...
    // Configure - SAME settings as TX!
    DW1000.newConfiguration();
    DW1000.setDefaults();
    DW1000.setDeviceAddress(2);  // Different address
    DW1000.setNetworkId(10);     // Same network
    ...
    Serial.println("\nListening for PING messages (polling mode)...\n");
}
...
void loop() {
    ...
    if (status & (1UL << RXFCE_BIT)) {
        errCount++;
        Serial.println("[ERR] CRC error detected");
        startReceiver();
    }
}
